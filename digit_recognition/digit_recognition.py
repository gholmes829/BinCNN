"""
MNIST handwritten digit classification with BinCNN. Expects GPU hardware.
"""
...
def showImage(image: np.ndarray) -> None:
    """Displays image in grayscale"""
    plt.imshow(image, cmap="gray")

def plotCosts(loss: list, accuracy: list, name: str) -> None:
...
def makeBinCNN():
    ...
    kwargs = dict(input_quantizer="ste_sign",  # critical! Do not change unless you know what you are doing.
    ...
    model.add(tf.keras.layers.Activation("softmax"))  # interpret output as probabilities

    # compiling

    model.compile(optimizer='adam',
                  loss='sparse_categorical_crossentropy',  # good for classification
                  metrics=['accuracy'])
...
def train_and_evaluate(model,
                       ...
                       name: str = "model", epochs: int = 10, batch_size: int = 64, verbose: bool = True) -> list:
    """Trains and tests a given model. Returns training loss and accuracy as list of each respectively for each epoch."""
    print("-"*100)
...
@profile
def feedForwardBinCNN(model, data: np.ndarray, labels: np.ndarray) -> None:
    """Feed forward for profiling purposes"""
    model.evaluate(data, labels, verbose=0)

@profile
def feedForwardCNN(model, data: np.ndarray, labels: np.ndarray) -> None:
    """Feed forward for profiling purposes"""
    model.evaluate(data, labels, verbose=0)
...
def main() -> None:
    ...
    """
    # FOLLOWING PRINT STATEMENTS ALSO HELP DESCRIBE CODE
